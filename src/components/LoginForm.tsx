...
export default function LoginForm() {
  ...
  const login = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = {
      username: formData.get('usuario')?.toString() || '',
      senha: formData.get('senha')?.toString() || '',
    };

    const result = await signIn("credentials", {
      ...data,
      redirect: false, // Não redireciona automaticamente, controlamos o redirecionamento manualmente
    });

    if (result?.error) {
      console.log(result.error); // Mostra o erro no console para depuração
